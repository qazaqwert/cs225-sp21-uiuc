...
public:
  ...
  InorderTraversal(typename BinaryTree<T>::Node *root)
      : root(root)
  {
    //iterate over tree and store left side to produce traversal
  ...
  void add(typename BinaryTree<T>::Node *&treeNode)
  {
    ...
    {
      //if right side isn't null, iterate over right side
      typename BinaryTree<T>::Node *curr = treeNode->right;

      //find appropriate slot to add data so order is maintained
      while (curr != NULL)
      {
        stack.push(curr);
        curr = curr->left;
