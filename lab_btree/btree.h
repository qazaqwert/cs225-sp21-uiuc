...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    //if tree is empty or val is smallest element, we put it
    //on the top of daTree
    if (elements.size() == 0 || val < elements[0])
        return 0;

    //if val is largest element, insert it at the end
    if (val > elements[elements.size() - 1])
        return elements.size();

    //iterate over daElements
    short smallChungus = 0;
    //also was i supposed to do this recursively lmak
    for (unsigned long i = 0; i < elements.size(); i++){
        smallChungus += (short)(i);

        //we found daIndex to insert at
        //std::cout << "in for loop before if" << std::endl;
        if (elements[i] > val || elements[i] == val)
            return i;
        //std::cout << "in for loop after if" << std::endl;
    }

    //if we got here then we do certifiably be done, got em
    return elements.size();
}
