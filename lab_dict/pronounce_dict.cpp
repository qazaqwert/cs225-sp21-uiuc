...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    auto proGamer1 = dict.find(upper1);
    auto proGamer2 = dict.find(upper2);

    if (dict.end() == proGamer2) return false;
    if (dict.end() == proGamer1) return false;
    if (proGamer2->second != proGamer1->second) return false;
