...
TEST_CASE("List::triplerotate simple", "[weight=10][part=1][valgrind]") {
    ...
    for (int i = 1; i <= 6; i++)
        list.insertBack(i);
    //cout << "Before triplerotate" << endl;
    list.tripleRotate();
    //cout << "After tripleroate" << endl;
    stringstream s;
...
TEST_CASE("List::ListIterator::end is not ::begin in a non-empty list", "[weight=1][part=1][valgrind]") {
    List<unsigned> list;
    //cout << "before insertfront 1" << endl;
    list.insertFront(1);
    //cout << "before insertfront 2" << endl;
    list.insertFront(2);
    //cout << "before insertfront 3" << endl;
    list.insertFront(3);
    //cout << "after insertfront 3" << endl;
    REQUIRE( (bool)(list.begin() != list.end()) );
    //cout << "after REQUIRE" << endl;
}
