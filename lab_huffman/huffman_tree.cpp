...
void HuffmanTree::buildTree(const vector<Frequency> &frequencies)
{
    ...
    {
        TreeNode *ptr = new TreeNode(f);
        singleQueue.push(ptr);
    }
    while (singleQueue.size() + mergeQueue.size() > 1)
    {
        ...
        if (second != NULL)
        {
            secondFrequency = second->freq.getFrequency();
        }
        TreeNode *merged = new TreeNode(first->freq.getFrequency() + secondFrequency);
        merged->left = first;
        merged->right = second;
