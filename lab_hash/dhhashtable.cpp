...
void DHHashTable<K, V>::remove(K const& key)
{
     ...
     * @todo Implement this function
     */
    int daIndex = findIndex(key);

    if (daIndex == -1) return;
    
    if (daIndex != -1) {
        --elems;
        delete table[daIndex];
        table[daIndex] = NULL;
    }
}
...
int DHHashTable<K, V>::findIndex(const K& key) const
{
    ...
    //very similar to t he other one lol
    size_t daIndex = hashes::hash(key, size);
    size_t daTemp = daIndex;
    while (should_probe[daIndex]) {
        //if i swap this order my foreskin will regrow
        if (table[daIndex] != NULL && table[daIndex]->first == key) return daIndex;
        daIndex = (daIndex + hashes::secondary_hash(key, size)) % size;

        //da key is not there and we are sadge
        if (daIndex == daTemp) break;
    }
