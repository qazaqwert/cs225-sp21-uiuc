...
private:
  ...
  /**
     ...
     */
  KDTreeNode * treeRecurse(vector<Point<Dim>> & newPoints, int left, int right, int d);

  Point<Dim> & quickSel(vector<Point<Dim>> & newPoints, int left, int right, int d, int i);

  int partition(vector<Point<Dim>> & newPoints, int left, int right, int d, int i);

  KDTreeNode * copyRecurse(KDTreeNode * other, KDTreeNode * sub);

  void NNRecurse(Point<Dim> &currBest, KDTreeNode *currNode, Point<Dim> &query, int delta) const;

  void KDTDestroy(KDTreeNode * & sub);
};
